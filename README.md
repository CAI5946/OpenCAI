...
- 普通文本：发送给当前 execution mode。默认 `agent` mode 走 Agent Loop；`guided` mode 先运行 Clarify，再把 `DemandBrief` 注入普通 Agent Loop；`workflow` mode 走 Workflow Clarify / Planner / WorkflowRunner。
...
- `/mode guided`：切换到 guided mode，普通文本会先经过 Clarify，再按 `DemandBrief` 进入普通 Agent Loop。
