...
- Modes：实现 guided mode 的 Clarify / DemandBrief / Agent Loop 执行流，再设计 Runtime-level `ModeProfile`，评估 learn / dev / debug / review mode 如何影响 prompt、workflow selection、strategy selection 和 tool policy。
...
- `python -m unittest discover tests`：exit code `0`，225 个测试通过。
