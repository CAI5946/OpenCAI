...
OpenCAI 的目标已调整为设计并开发完整成熟的 Coding Agent。后续路线继续采用 Feature Epic + 小切片，但小切片只是交付和验证方式，不再代表产品目标只能停留在最小方案。Workflow、Multi-agents、Agent Loop Strategy 是三条主 feature；Modes、Streaming Outputs、LLM Council、Context Engineering 是新增候选 feature，按架构影响和依赖逐步评估。
...
后续 feature 依赖关系：Context Engineering 是 Runtime / Agent Loop / Workflow prompt composition 的输入合同，应先解决普通 Agent Loop 只接收 `user_task`、缺少 Session 初始化 context 的缺口；Multi-agents 依赖 Workflow 的 state、dispatcher、aggregator 和 scoped context；Agent Loop Strategy 属于 benchmark-driven experiment，不应打断 Workflow 主线；Modes 应先作为 Runtime-level `ModeProfile`；Streaming Outputs 需要 event iterator / sink 兼容现有 list-return 路径；LLM Council 应先做 role-based model routing，不急着做投票型 council。
...
- Feature G / Context Engineering：设计 Session 初始化 context，定义 `ContextSnapshot` / `ContextProvider` / `ContextComposer` 的最小边界，先不做传统 RAG 或复杂长期 memory。
