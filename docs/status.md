...
- `OpenCAI/demand.py` 已定义 guided / workflow 可复用的 `DemandBrief` 执行合同和 `<demand_brief>` 渲染格式；当前尚未接入 `ContextComposer` 或 guided 执行路径。
...
- Modes：把 `DemandBrief` 接入 `ContextComposer`，再实现 guided mode 的 Clarify / DemandBrief / Agent Loop 执行流；之后再设计 Runtime-level `ModeProfile`，评估 learn / dev / debug / review mode 如何影响 prompt、workflow selection、strategy selection 和 tool policy。
...
- `python -m unittest discover tests`：exit code `0`，230 个测试通过。
