...
- `.env`、`.opencai/models.json` 和 `.codex/config.toml` 作为本地配置排除版本控制，只保留无密钥示例。
- 公开定位是 Alpha 阶段 CLI Coding Agent 产品原型，不把长期路线描述为已完成能力。
- 公开仓库必须使用已清理历史，不得直接将包含无再分发许可证源码快照的旧私有仓库切换为 Public。
...
- `2026-07-13` 开源候选验证：`python -m unittest discover tests` 运行 334 个测试并通过；CLI version / help / dry-run smoke 通过。
- 公开配置示例通过 JSON / TOML / YAML 解析；公开文件树和清理后 Git 历史必须在发布前分别通过 Gitleaks 0 命中扫描。
