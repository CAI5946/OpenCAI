...
- 默认 agent mode 下普通 task 走 Agent Loop；guided mode 下普通 task 先走 Clarify / DemandBrief，再进入 Agent Loop；workflow mode 下普通文本自动走当前内置 `inspect -> handoff` workflow；`/workflow TASK` 保留为显式 workflow 入口。
- `--task` 保留为一次性调试路径。
...
- TUI 已具备 slash command、`$skill` 显式 skill 调用入口、`!` shell mode、`/model` 二级选择、`/mode agent|guided|workflow`、`/keymap`、prompt history、多行输入、状态栏、执行中 live process 和最近一次 task 过程视图；`Shift+Tab` 当前切换 execution mode，permission 通过 `/permission` 切换。
- `OpenCAI/demand.py` 已定义 guided / workflow 可复用的 `DemandBrief` 执行合同和 `<demand_brief>` 渲染格式；`ContextComposer.compose(..., demand_brief=...)` 已能在 session context 后、当前 task 前注入该合同；`OpenCAI/guided.py` 当前负责 `ClarifyResult -> DemandBrief` 转换和 guided orchestration。
...
- Modes：完善 guided mode 的 Clarify 结果展示、blocked 行为和后续策略；之后再设计 Runtime-level `ModeProfile`，评估 learn / dev / debug / review mode 如何影响 prompt、workflow selection、strategy selection 和 tool policy。
...
- `python -m unittest tests.test_workflow_clarify tests.test_guided tests.test_runtime_session tests.test_workflow_commands tests.test_context tests.test_demand`：exit code `0`，45 个测试通过，确认 guided mode 复用 clarify、传入 session context、转换 `DemandBrief` 并路由到 `run_once(..., demand_brief=...)`。
- `cmd /c "(echo /mode guided&echo Read README&echo /exit)|python -m OpenCAI --adapter fake --max-steps 3"`：exit code `0`，确认 guided mode 在 fake adapter 下 clarify complete 后进入普通 Agent Loop。
- `python -m unittest discover tests`：exit code `0`，240 个测试通过。
