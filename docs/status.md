...
- `OpenCAI/demand.py` 已定义 guided / workflow 可复用的 `DemandBrief` 执行合同和 `<demand_brief>` 渲染格式；`ContextComposer.compose(..., demand_brief=...)` 已能在 session context 后、当前 task 前注入该合同；`OpenCAI/guided.py` 当前负责 `ClarifyResult -> DemandBrief` 转换、DemandBrief preview 和 guided review gate。
...
- Modes：完善 guided mode 的 blocked 行为、review feedback 策略和过程可观察性；之后再设计 Runtime-level `ModeProfile`，评估 learn / dev / debug / review mode 如何影响 prompt、workflow selection、strategy selection 和 tool policy。
...
- `python -m unittest tests.test_guided tests.test_runtime_session tests.test_workflow_clarify tests.test_workflow_commands tests.test_context tests.test_demand`：exit code `0`，50 个测试通过，确认 guided mode 复用 clarify、传入 session context、转换 `DemandBrief`、支持 review feedback 并路由到 `run_once(..., demand_brief=...)`。
...
- `python -m unittest discover tests`：exit code `0`，245 个测试通过。
