...
- 普通 task 路径已支持 event streaming：Agent Loop 通过 `iter_agent_loop()` 边产生边交付 transcript events，Runtime 交给 TUI 即时渲染；`run_agent_loop()` 继续保留 `list[Event]` 兼容测试、workflow 和 benchmark。
