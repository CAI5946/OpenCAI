...
- TUI 已具备 slash command、`!` shell mode、`/model` 二级选择、suggestion key binding、状态栏、轻量 composer 输入区和 `/process` 最近一次 task 临时过程视图。
- 普通 task 路径已支持 event streaming 数据源：Agent Loop 通过 `iter_agent_loop()` 产生 transcript events；Runtime 默认渲染折叠摘要，只显示 final answer，完整过程保存在 session 中并可通过 `/process` 展开；TTY 交互下 `/process` 使用临时全屏视图，按 `Esc` / `Enter` / `q` 收起；`run_agent_loop()` 继续保留 `list[Event]` 兼容测试、workflow 和 benchmark。
