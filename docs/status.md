...
当前主线是 Feature A: Workflow。WorkflowSpec / WorkflowRunner 已完成首个串行 runtime 切片和 `/workflow` CLI 入口；`/workflow TASK` 已从普通 runtime command 中分流为结构化 `WorkflowCommandInput`，并由 `workflow_commands.py` 承接 workflow command flow；`workflow_planner.py` 已提供 Planner / Compiler V1 边界；Workflow Core Model 已引入 `WorkflowTask` / `TaskResult`，当前内置 `inspect_handoff` workflow 是 2 个 phase / 3 个 task 的串行 task graph。下一步继续补 task-scoped context / policy，而不是先做一次性的 confirmation gate。
...
- TUI 已具备 slash command、`$skill` 显式 skill 调用入口、`!` shell mode、`/model` 二级选择、`/keymap` 只读快捷键弹窗、suggestion key binding、prompt history、`Shift+Enter` / `Ctrl+J` 多行输入、状态栏、轻量 composer 输入区、执行中 live process 和最近一次 task 临时过程视图；`Shift+Enter` 已兼容 Windows console Shift+Enter 事件，以及 `ESC[13;2u` 和 `ESC[27;2;13~` 两类常见 modified Enter 序列。已确认不要在 prompt_toolkit 路径主动开启 terminal keyboard enhancement / modified-key reporting，因为当前终端会把 CSI-u 内容注入输入缓冲。
...
- Command parser 已将 `/workflow TASK` 识别为 `WorkflowCommandInput`；`run_interactive()` 会直接分发到 `workflow_commands.py`，普通 slash command 仍走 `RuntimeCommandInput` / `handle_runtime_command()`。
- Workflow Planner / Compiler V1 已拆为 `workflow_planner.py`，当前 `compile_workflow(task)` 返回内置 `inspect_handoff` `WorkflowSpec`，为后续模板选择和 WorkflowScript 生成保留边界。
- WorkflowRunner 当前支持串行 task 执行、task dependency 检查、prompt composition、`TaskResult` 聚合为 `PhaseResult`，并通过 `final_phase_id` 的单 task phase 生成 workflow final answer。
...
- Feature A / Workflow：继续跑通核心链路，下一步设计 dependency-aware task context 和 task / phase scoped policy。
- Feature A / Workflow：`workflow_commands.py` 已拆出第一刀；execute / cancel 交互后续通过统一 human decision / control point 处理。
- Feature A / Workflow：`workflow_planner.py` 已拆出第一刀；`WorkflowTask` / `TaskResult` 和多 task phase aggregation 已完成第一版。
...
- 当前只有内置 `inspect -> handoff` workflow；该 workflow 已是 2 phase / 3 task 的串行 task graph，但尚未接 Nodeflow bugfix workflow、retry loop、humancheck、save/replay 或 LLM-generated WorkflowSpec / WorkflowScript。
...
- `python -m unittest discover tests`：exit code `0`，200 个测试通过，确认 Workflow Task Graph 第一版、`workflow_plan` tool tasks 输出和现有 runtime / workflow / context / safety / tooling 路径全量未回归。
- `python -m unittest tests.test_workflow tests.test_workflow_planner tests.test_workflow_commands tests.test_tool_taxonomy`：exit code `0`，19 个测试通过，确认 `WorkflowTask` / `TaskResult`、多 task phase aggregation、planner 模板和 `workflow_plan` tool 结构正常。
- `python -m py_compile OpenCAI\workflow.py OpenCAI\workflow_planner.py OpenCAI\workflow_commands.py OpenCAI\tooling\workflow_tools.py tests\test_workflow.py tests\test_workflow_planner.py tests\test_workflow_commands.py tests\test_tool_taxonomy.py`：exit code `0`，确认 workflow core、planner、command flow、workflow tool 和测试语法可编译。
- `cmd /c "(echo /workflow Read README&echo /exit)|python -m OpenCAI --adapter fake --max-steps 3"`：exit code `0`，确认非 TTY `/workflow Read README` 会展示 phases / tasks plan，执行 3 个 task，并聚合 `inspect` phase 的 2 个 task result。
- `cmd /c "(echo /workflow&echo /exit)|python -m OpenCAI --adapter fake --max-steps 3"`：exit code `0`，确认空 workflow task 仍只输出 `No task for workflow. Usage: /workflow TASK`，未执行 WorkflowRunner。
- `python -m unittest discover tests`：exit code `0`，199 个测试通过，确认 Workflow Planner V1 接入后现有 runtime / workflow / context / safety / tooling 路径全量未回归。
- `python -m unittest tests.test_composer tests.test_runtime_session tests.test_runtime_commands tests.test_workflow_commands tests.test_workflow_planner tests.test_workflow`：exit code `0`，60 个测试通过，确认 Workflow Planner V1、workflow command flow、parser 分流和现有 WorkflowRunner 行为未回归。
- `python -m py_compile OpenCAI\workflow_planner.py OpenCAI\workflow_commands.py tests\test_workflow_planner.py tests\test_workflow_commands.py`：exit code `0`，确认 planner / command flow 新增模块和测试语法可编译。
- `cmd /c "(echo /workflow Read README&echo /exit)|python -m OpenCAI --adapter fake --max-steps 3"`：exit code `0`，确认非 TTY `/workflow Read README` 经 planner 编译后仍能执行内置 `inspect -> handoff` workflow。
- `cmd /c "(echo /workflow&echo /exit)|python -m OpenCAI --adapter fake --max-steps 3"`：exit code `0`，确认空 workflow task 仍只输出 `No task for workflow. Usage: /workflow TASK`，未执行 WorkflowRunner。
- `python -m unittest discover tests`：exit code `0`，197 个测试通过，确认 `/workflow` parser 分流、`workflow_commands.py` 拆分和现有 runtime / workflow / context / safety / tooling 路径全量未回归。
- `python -m unittest tests.test_composer tests.test_runtime_session tests.test_runtime_commands tests.test_workflow_commands`：exit code `0`，48 个测试通过，确认 `/workflow TASK` parser 分类、交互 runtime 分发、`workflow_commands.py` 空 task 提示、旧 runtime command 兼容委托和普通 command 路径未回归。
- `python -m py_compile OpenCAI\composer.py OpenCAI\__main__.py OpenCAI\runtime_commands.py OpenCAI\workflow_commands.py tests\test_composer.py tests\test_runtime_session.py tests\test_runtime_commands.py tests\test_workflow_commands.py`：exit code `0`，确认 command parser、workflow command flow 和测试语法可编译。
- `cmd /c "(echo /workflow Read README&echo /exit)|python -m OpenCAI --adapter fake --max-steps 3"`：exit code `0`，确认非 TTY `/workflow Read README` 经 `WorkflowCommandInput` 分发后仍能执行内置 `inspect -> handoff` workflow。
- `cmd /c "(echo /workflow&echo /exit)|python -m OpenCAI --adapter fake --max-steps 3"`：exit code `0`，确认空 workflow task 输出 `No task for workflow. Usage: /workflow TASK`，未执行 WorkflowRunner。
- `python -m unittest tests.test_tui_status_bar`：exit code `0`，31 个测试通过，确认回滚 terminal keyboard enhancement 后，Shift+Enter 兼容路径、Ctrl+J multiline、状态栏和快捷键 handoff 未回归。
- `python -m py_compile OpenCAI\tui.py tests\test_tui_status_bar.py`：exit code `0`，确认 TUI Shift+Enter 兼容代码和测试语法可编译。
- `python -m unittest discover tests`：exit code `0`，191 个测试通过，确认 `/keymap`、TUI history / Shift+Enter (Windows console event、`ESC[13;2u` 和 `ESC[27;2;13~`) / Ctrl+J multiline / 多行输入可见性 / 快捷键 handoff 和现有 runtime / workflow / context / safety / tooling 路径全量未回归。
