...
- `OpenCAI/demand.py` 已定义 guided / workflow 可复用的 `DemandBrief` 执行合同和 `<demand_brief>` 渲染格式；`ContextComposer.compose(..., demand_brief=...)` 已能在 session context 后、当前 task 前注入该合同；当前尚未接入 guided 执行路径。
...
- Modes：实现 guided mode 的 Clarify / DemandBrief / Agent Loop 执行流；之后再设计 Runtime-level `ModeProfile`，评估 learn / dev / debug / review mode 如何影响 prompt、workflow selection、strategy selection 和 tool policy。
...
- `python -m unittest discover tests`：exit code `0`，232 个测试通过。
