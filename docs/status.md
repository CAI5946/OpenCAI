...
- Product Goal / Benchmark：Level 1 本地 micro benchmark 已升级为诊断报告合同；下一刀应扩大任务集或把同一 benchmark 接入 Workflow/Mode 策略对比，而不是先做 Multi-agent。
...
- Benchmark：扩展 Level 1 micro tasks 到 10 个，或新增 benchmark mode 参数用于对比 plain Agent Loop 与 Workflow-guided bugfix loop。
...
- `python -m benchmarks.runner --task all --timeout 30`：exit code `1`，fake adapter baseline 为 `0/3 passed`；报告正常生成，三个任务均保留初始失败证据并输出 `failed_verification` 诊断状态。
