...
- 准备进入 Phase 4：Agent Loop。

## 下一步

- 先说明 Agent Loop 的职责、输入、输出、状态、失败情况和边界。
- 区分 Runtime state、Event / Transcript、ToolResult observation 和 LLM messages。
- 设计不接真实 LLM 的最小 fake loop。
- 在用户确认理解后，再做最小 Agent Loop 实现。
