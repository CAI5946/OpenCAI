...
8. Skill tools
   ...
   Skill tools 是本地可复用能力包入口，和 MCP / plugin 不完全相同。MCP 偏外部工具协议和资源读取，plugin 偏扩展分发和动态能力安装，skill 偏本地规则、模板、脚本和 workflow 经验复用。后续可以通过 `tool_search` 延迟发现 skill，但执行边界应单独建模。
