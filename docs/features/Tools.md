...
8. Skill tools
   ...
   Skill tools 是本地可复用能力包入口，和 MCP / plugin 不完全相同。MCP 偏外部工具协议和资源读取，plugin 偏扩展分发和动态能力安装，skill 偏本地规则、模板、脚本和 workflow 经验复用。后续可以通过 `tool_search` 延迟发现 skill，但执行边界应单独建模。`invoke_skill` 的语义是加载 skill 指令并注入 messages / context，不等同于直接执行任意脚本。
...
### Skill / MCP / dynamic tools

后续补：

- `invoke_skill`：读取 SkillRegistry，展开 `SKILL.md`，返回 injected messages / context delta。
...
- Context 层当前会把 `<repo>/.opencai/skills` 和 `~/AgentSkills` 中的 skill 摘要注入 `<available_skills>`，但只包含 name 和 description，不在常规 model context 中暴露完整路径或 source。
- `invoke_skill` 尚未实现；当前 skill registry summary 只让模型知道可用技能，不会自动加载完整 `SKILL.md`。
