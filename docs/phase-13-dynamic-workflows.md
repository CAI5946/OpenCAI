...
- 第一版范围：实现最小串行 `WorkflowRunner` 和 manifest，先用内置 workflow function 模拟脚本式编排，验证 workflow 层可以稳定编排多个 phase。
...
第一版不做完整脚本 runtime，但实现方向要靠近脚本 runtime：先抽出 Runner API 和 `run_phase` 思维，避免把声明式 schema 设计得过重。
...
第一版只保留最小失败停止；retry 放到 Workflow feature 的后续切片。
...
Phase 13 第一版只做最小串行 workflow runtime，但方向上靠近脚本式 runtime。
...
第一版不需要解析外部脚本文件，可以先用内置 workflow function 或最小 template 模拟脚本控制流。
...
目标是在不替换 Runtime、LLMAdapter、Tool Model、Event / Transcript 和 Verification 协议的前提下，抽象最小 strategy 接口，对比：
